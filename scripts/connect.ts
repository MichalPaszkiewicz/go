...
module Go {
    ...
    declare class Peer {
        on(thing: string, fn: any)
        connect(stringy: string, thing: any);
        connections: Object;
        destroyed: boolean;
        destroy: () => void;
    ...
    peer.on('open', function (id) {
        ...
        if (location.search.length == 0) {
            ...
            PlayerService.Instance.setBlack();
        }
    });
        ...
        if (c.label === 'chat') {
            ...
            c.on('data', function (data) {
                ...
                if (data.indexOf("goMove=") > -1) {
                    ...
                    addMove(xPos, yPos, PlayerService.Instance.otherPlayer());
                    ...
                    GameService.Instance.isHost = false;
                    ...
                    GameService.Instance.setColour(parseInt(settingsString.substring(settingsString.indexOf("c") + 1)));

                    PlayerService.Instance.player = sentPlayer;
    ...
    $(document).ready(function () {
        ...
        $("#games-table").on('click', 'td', function () {
            ...
            addMove(xPos, yPos, PlayerService.Instance.player);
    ...
    export function sendSettings() {
        var msg = "goSettings=" + "s" + GameService.Instance.board.length
            + "p" + PlayerService.Instance.player
            + "c" + GameService.Instance.Colour;
